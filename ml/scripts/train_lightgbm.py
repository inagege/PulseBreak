...
def build_lightgbm(cfg: dict, *, random_state: int):
    try:
        from lightgbm import LGBMClassifier
    except ImportError as exc:
        raise RuntimeError("lightgbm is not installed. Install ml/requirements.txt before training this model.") from exc
...
def run() -> None:
    parser = argparse.ArgumentParser(description="Train a LightGBM stress model and evaluate it on the fixed subject holdout.")
    parser.add_argument("--config", default="ml/configs/stress_lightgbm.yaml")
    ...
    for repeat in range(repeats):
        ...
        for fold_idx, (train_rel, val_rel) in enumerate(cv.split(x_dev, y_dev, groups=groups_dev), start=1):
            ...
            clf = build_lightgbm(tr_cfg, random_state=seed + repeat * 100 + fold_idx)
    ...
    clf_final = build_lightgbm(tr_cfg, random_state=seed)
    ...
    bundle = {
        ...
        "selected_params": {
            ...
            "num_leaves": int(tr_cfg.get("num_leaves", 15)),
            "max_depth": int(tr_cfg.get("max_depth", -1)),
            "min_child_samples": int(tr_cfg.get("min_child_samples", 20)),
            ...
            "reg_lambda": float(tr_cfg.get("reg_lambda", 0.0)),
    ...
    print(f"[lightgbm] Saved bundle: {bundle_path}")
    print(f"[lightgbm] Selected params: {bundle['selected_params']}")
    print(f"[lightgbm] Dev OOF metrics: {dev_oof_metrics}")
